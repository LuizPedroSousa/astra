...
void ConsolePanelController::refresh(bool force) {
  ...
  if (force || entries_changed) {
    ...
    const float indicator_width =
        measure_text_width(style_constraints.meta_font_size, ">");
    ...
    for (size_t source_index = 0u; source_index < entries.size();
      ...
      if (!panel::matches_severity_filter(entry, m_severity_filter) ||
      ...
      visible_entry.expanded =
          ...
          (m_expand_all_details ||
           (m_expanded_source_index.has_value() &&
...
ui::UINodeId ConsolePanelController::create_row_slot(size_t slot_index) {
  ...
  RowNodes row_nodes;
  row_nodes.slot = m_document->create_view("console_row_slot_" + suffix);
  ...
  row_nodes.disclosure = ui::create_disclosure(
      *m_document,
      ...
      ui::UIDisclosureOptions{
          .name = "console_row_disclosure_" + suffix,
          .open = false,
      }
  );
  ...
  row_nodes.indicator =
      m_document->create_text({}, "console_row_indicator_" + suffix);
  row_nodes.meta = m_document->create_text({}, "console_row_meta_" + suffix);
  row_nodes.badge =
      m_document->create_text({}, "console_row_badge_" + suffix);
  row_nodes.primary =
      m_document->create_text({}, "console_row_primary_" + suffix);
  row_nodes.secondary =
      m_document->create_text({}, "console_row_secondary_" + suffix);
  ...
  m_document->mutate_style(row_nodes.indicator, [this](ui::UIStyle &style) {
    ...
    style.text_color = panel::alpha(k_theme.bunker_300, 0.92f);
  });
...
void ConsolePanelController::bind_row_slot(
  ...
  m_document->set_text(
      row_nodes.indicator, panel::disclosure_indicator(expandable, expanded)
  );

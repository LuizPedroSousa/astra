...
public:
  ...
  uint32_t declare_framebuffer(const std::string &name, FramebufferSpecification spec, RenderGraphResourceLifetime lifetime = RenderGraphResourceLifetime::Transient) {
  ...
  PassBuilder
  add_pass(Scope<RenderPass> pass, RenderGraphPassType type = RenderGraphPassType::Graphics);
...
inline PassBuilder RenderGraphBuilder::add_pass(Scope<RenderPass> pass, RenderGraphPassType type) {

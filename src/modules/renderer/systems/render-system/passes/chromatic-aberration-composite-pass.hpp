...
class ChromaticAberrationCompositePass : public FramePass {
public:
  explicit ChromaticAberrationCompositePass(
      rendering::ResolvedMeshDraw fullscreen_quad = {}
  )
      : m_fullscreen_quad(std::move(fullscreen_quad)) {}
  ~ChromaticAberrationCompositePass() override = default;

  void setup(PassSetupContext &ctx) override {
    m_shader = ctx.require_shader("chromatic_aberration_composite_shader");

    if (m_shader == nullptr) {
      LOG_WARN(
          "[ChromaticAberrationCompositePass] Missing graph dependency: "
          "chromatic_aberration_composite_shader"
      );
      set_enabled(false);
    }
  }
  ...
  void record(PassRecordContext &ctx, PassRecorder &recorder) override {
    ASTRA_PROFILE_N("ChromaticAberrationCompositePass::record");
    ...
    if (chromatic_aberration_resource == nullptr ||
        present_resource == nullptr || m_shader == nullptr ||
        m_fullscreen_quad.vertex_array == nullptr ||
        m_fullscreen_quad.index_count == 0) {
      return;
    }

    using namespace shader_bindings::
        engine_shaders_chromatic_aberration_composite_axsl;

    auto &frame = ctx.frame();
    const auto extent = ctx.graph_image_extent(*present_resource);

    auto effect_input = ctx.register_graph_image(
        "chromatic-aberration-composite.effect-input",
        *chromatic_aberration_resource
    );
    auto present_target = ctx.register_graph_image(
        "chromatic-aberration-composite.present-target", *present_resource
    );

    const auto bindings = frame.register_binding_group(
        make_binding_group_desc(
            "chromatic-aberration-composite-pass",
            "chromatic-aberration-composite-pass",
            m_shader,
            0,
            "chromatic-aberration-composite-pass",
    ...
    frame.add_sampled_image_binding(
        bindings,
        ChromaticAberrationCompositeResourcesResources::effect_input
            .binding_id,
        ImageViewRef{.image = effect_input}
    );

    RenderPipelineDesc pipeline_desc;
    pipeline_desc.debug_name = "chromatic-aberration-composite-pass";
    ...
    const auto quad_buffer = frame.register_vertex_array(
        "chromatic-aberration-composite-pass.fullscreen-quad",
        m_fullscreen_quad.vertex_array
    );

    RenderingInfo info;
    info.debug_name = "chromatic-aberration-composite-pass";
    info.extent = extent;
  ...
  std::string name() const override {
    return "ChromaticAberrationCompositePass";
  }

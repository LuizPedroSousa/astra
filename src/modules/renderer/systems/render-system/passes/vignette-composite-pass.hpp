...
class VignetteCompositePass : public FramePass {
public:
  explicit VignetteCompositePass(
      rendering::ResolvedMeshDraw fullscreen_quad = {}
  )
      : m_fullscreen_quad(std::move(fullscreen_quad)) {}
  ~VignetteCompositePass() override = default;

  void setup(PassSetupContext &ctx) override {
    m_shader = ctx.require_shader("vignette_composite_shader");

    if (m_shader == nullptr) {
      LOG_WARN(
          "[VignetteCompositePass] Missing graph dependency: "
          "vignette_composite_shader"
      );
      set_enabled(false);
    }
  }
  ...
  void record(PassRecordContext &ctx, PassRecorder &recorder) override {
    ASTRA_PROFILE_N("VignetteCompositePass::record");
    ...
    frame.add_sampled_image_binding(
        bindings,
        VignetteCompositeResourcesResources::effect_input.binding_id,
        ImageViewRef{.image = effect_input}
    );

    RenderPipelineDesc pipeline_desc;
    pipeline_desc.debug_name = "vignette-composite-pass";
    ...
    const auto quad_buffer = frame.register_vertex_array(
        "vignette-composite-pass.fullscreen-quad",
        m_fullscreen_quad.vertex_array
    );

    RenderingInfo info;
    info.debug_name = "vignette-composite-pass";
    info.extent = extent;
  ...
  std::string name() const override { return "VignetteCompositePass"; }

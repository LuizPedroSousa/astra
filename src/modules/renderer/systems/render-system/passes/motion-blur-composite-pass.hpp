...
class MotionBlurCompositePass : public FramePass {
public:
  explicit MotionBlurCompositePass(
      rendering::ResolvedMeshDraw fullscreen_quad = {}
  )
      : m_fullscreen_quad(std::move(fullscreen_quad)) {}
  ~MotionBlurCompositePass() override = default;

  void setup(PassSetupContext &ctx) override {
    m_shader = ctx.require_shader("motion_blur_composite_shader");

    if (m_shader == nullptr) {
      LOG_WARN(
          "[MotionBlurCompositePass] Missing graph dependency: "
          "motion_blur_composite_shader"
      );
      set_enabled(false);
    }
  }
  ...
  void record(PassRecordContext &ctx, PassRecorder &recorder) override {
    ASTRA_PROFILE_N("MotionBlurCompositePass::record");
    ...
    if (motion_blur_resource == nullptr || scene_color_resource == nullptr ||
        m_shader == nullptr || m_fullscreen_quad.vertex_array == nullptr ||
        m_fullscreen_quad.index_count == 0) {
      return;
    }

    using namespace shader_bindings::engine_shaders_motion_blur_composite_axsl;

    auto &frame = ctx.frame();
    const auto extent = ctx.graph_image_extent(*scene_color_resource);

    auto motion_blur_input = ctx.register_graph_image(
        "motion-blur-composite.motion-blur-input", *motion_blur_resource
    );
    auto scene_color_target = ctx.register_graph_image(
        "motion-blur-composite.scene-color-target", *scene_color_resource
    );

    const auto bindings = frame.register_binding_group(
        make_binding_group_desc(
            "motion-blur-composite-pass",
            "motion-blur-composite-pass",
            m_shader,
            0,
            "motion-blur-composite-pass",
    ...
    frame.add_sampled_image_binding(
        bindings,
        MotionBlurCompositeResourcesResources::motion_blur_input.binding_id,
        ImageViewRef{.image = motion_blur_input}
    );

    RenderPipelineDesc pipeline_desc;
    pipeline_desc.debug_name = "motion-blur-composite-pass";
    ...
    const auto quad_buffer = frame.register_vertex_array(
        "motion-blur-composite-pass.fullscreen-quad",
        m_fullscreen_quad.vertex_array
    );

    RenderingInfo info;
    info.debug_name = "motion-blur-composite-pass";
    info.extent = extent;
  ...
  std::string name() const override { return "MotionBlurCompositePass"; }

...
namespace astralix {
  ...
  namespace shader_detail {
    ...
    template <typename T> struct array_value_type;

    template <typename T, size_t N> struct array_value_type<std::array<T, N>> {
      using type = T;
    };
    ...
    template <typename T> struct flattened_value_count : std::integral_constant<size_t, 1> {};
  ...
  public:
    ...
    virtual void set_vec2(const std::string& name, glm::vec2 value) const = 0;
    virtual void set_vec3(const std::string& name, glm::vec3 value) const = 0;
    virtual void set_vec4(const std::string& name, glm::vec4 value) const = 0;

    template <typename FieldTag, typename Value>
      requires std::same_as<std::remove_cvref_t<Value>,
    typename FieldTag::value_type>
    ...
    (!shader_detail::is_std_array_v<
      ...
      shader_detail::array_value_type_t<
      typename FieldTag::value_type>>
  ...
  protected:
    ...
    virtual void set_typed_value(uint64_t binding_id, ShaderValueKind kind,
      const void* value) const = 0;

    ResourceDescriptorID m_descriptor_id;

  private:
    template <typename T> void upload_scalar_value(uint64_t binding_id,
      const T& value) const {
      set_typed_value(binding_id, shader_detail::value_kind<T>::kind, &value);
    }

    template <typename BindingIds, typename Value>
    size_t upload_value_recursive(const BindingIds& binding_ids, size_t index,
    ...
    void upload_field_value(const Value& value) const {
      ...
      static_assert(k_binding_count == k_value_count,
        "field tag binding count must match flattened value size");
    ...
    void upload_field_element_value(size_t index, const Value& value) const {
      ...
      static_assert(shader_detail::is_std_array_v<field_value_type>,
        "indexed field upload requires an array-valued field");
      static_assert(!shader_detail::is_std_array_v<element_value_type>,
        "indexed field upload only supports single-dimension arrays");
      static_assert(std::same_as<element_value_type, std::remove_cvref_t<Value>>,
        "indexed field upload value type mismatch");

...
namespace astralix {
  ...
  using GLSLExprPtr = std::unique_ptr<GLSLExpr>;
  using GLSLStmtPtr = std::unique_ptr<GLSLStmt>;
  ...
  using GLSLDecl =
    std::variant<GLSLStructDecl, GLSLGlobalVarDecl, GLSLInterfaceBlockDecl,
    GLSLFunctionDecl>;
  ...
  class GLSLLowering {
  public:
    GlslLoweringResult lower(const CanonicalStage& stage) const;
  };

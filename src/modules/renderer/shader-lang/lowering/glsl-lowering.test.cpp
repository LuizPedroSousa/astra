...
namespace astralix {
  ...
  namespace {
    ...
    CanonicalStage lower_canonical_stage(std::string_view source, StageKind stage) {
    ...
    GLSLStage lower_glsl_stage(std::string_view source, StageKind stage) {
      CanonicalStage canonical_stage = lower_canonical_stage(source, stage);
      GLSLLowering lowering;
      auto result = lowering.lower(canonical_stage);
      EXPECT_TRUE(result.ok());
      return std::move(result.stage);
    }

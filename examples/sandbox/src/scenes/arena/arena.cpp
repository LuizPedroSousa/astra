...
scene::Transform make_cube_transform(
    glm::vec3 position,
    glm::vec3 scale,
...
void emit_arena_layout(const SpawnFn &spawn_cube) {
  ...
  for (const auto &ramp_spec : ramp_specs) {
    spawn_cube(
        ...
        physics::RigidBodyMode::Static,
        ramp_spec.mass,
        ramp_spec.rotation_axis,
        ramp_spec.rotation_degrees
    );
  }
  ...
  for (const auto &cube_spec : dynamic_cube_specs) {
    spawn_cube(
        ...
        cube_spec.position,
        cube_spec.scale,
        physics::RigidBodyMode::Dynamic,
        cube_spec.mass,
        cube_spec.rotation_axis,
        cube_spec.rotation_degrees
    );
  }
}

void configure_cube_entity(
    ecs::EntityRef cube,
    glm::vec3 position,
    glm::vec3 scale,
    physics::RigidBodyMode mode,
    float mass,
    glm::vec3 rotation_axis,
    float rotation_degrees
) {
  const Mesh cube_mesh = Mesh::cube(cube_mesh_size);
  const auto transform = make_cube_transform(
      position, scale, rotation_axis, rotation_degrees
  );
...
void configure_meta_cube(
  ...
  auto cube = pass.entity(std::move(stable_key), std::move(name));
  cube.component(rendering::Renderable{});
  cube.component(rendering::ShadowCaster{});
...
void Arena::after_source_ready() { sync_spawned_cube_state(); }

void Arena::after_runtime_ready() { sync_spawned_cube_state(); }

void Arena::evaluate_build(SceneBuildContext &ctx) {
  auto pass = ctx.begin_pass("sandbox.arena");
  ...
  camera.component(make_transform(camera_position));
...
void Arena::sync_spawned_cube_state() {
...
void Arena::update_runtime() {
  ...
  while (m_spawn_cube_requests > 0u) {
    ...
    spawn_arena_cube(
        cube_name,
        glm::vec3(-2.0f + offset, 8.0f, 1.5f),
        glm::vec3(0.85f),
        physics::RigidBodyMode::Dynamic,
        1.0f
    );
...
void Arena::spawn_arena_cube(std::string name, glm::vec3 position, glm::vec3 scale, physics::RigidBodyMode mode, float mass, glm::vec3 rotation_axis, float rotation_degrees) {
  auto cube = spawn_scene_entity(std::move(name));
  configure_cube_entity(
      cube, position, scale, mode, mass, rotation_axis, rotation_degrees
  );
}
